...
- [~] Ingest the full UKLO corpus directly from https://www.uklo.org/past-exam-papers/ (PDFs of past papers + mark schemes). Check licensing/terms first; write a `source="uklo_pdf"` adapter for `normalize_raw`; upload raw PDFs to `gs://cot-rosetta-interp-data/raw/uklo_pdf/` and normalized JSONs to `gs://.../puzzles/`. The Phase 0 `source="uklo_txt"` ingest (23 puzzles from ef2020/PuzzleEvaluation) is a subset and a sanity check, not the corpus.
  - [x] **Raw scrape done.** 347 unique PDFs in `gs://cot-rosetta-interp-data/raw/uklo_pdf/` (~120 MB) via `scripts/scrape_uklo_pdfs.py` (Compute Engine VM in us-central1-a — uklo.org's SiteGround CAPTCHA blocks sandbox IPs but allows GCP IPs). Source index pages: `www.uklo.org/past-exam-papers/` (244 PDFs) and `archives.uklo.org/past-problems` (118 PDFs); 15 broken `archives.uklo.org/2019/03/*` links 404'd and were removed.
  - [ ] Crawl per-year archive index pages (`archives.uklo.org/papers-2014`, `problems-2016`, `problems-2017`, `22problems`, `previous-years`, `test-papers-2012`, etc.) for any PDFs not surfaced by the two top-level indexes; recover the missing 2019 R2 advanced set from a different path.
  - [ ] Identify and download the per-puzzle mark scheme / solution PDFs (only 4 explicitly named "Mark_Scheme" PDFs in the current scrape — the rest are bundled "Complete-Script" PDFs that need post-extraction to separate questions vs. solutions).
  - [ ] Write the `source="uklo_pdf"` adapter (`_normalize_uklo_pdf`) that PDF-extracts each file, identifies puzzles within multi-puzzle yearly bundles, classifies each by format, **filters to `format="Rosetta"`**, and writes one normalized JSON per puzzle to `gs://.../puzzles/`.
